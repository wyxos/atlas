...
function createReverbClient(config: ReverbConfig, pusherCtor: PusherCtor): ReverbClient | null {
...
async function connectReverbWithCtor(config: ReverbConfig, pusherCtor: PusherCtor): Promise<ReverbClient | null> {
    return createReverbClient(config, pusherCtor);
}

export {
    connectReverbWithCtor,
};
