...
class HttpRateLimiter
{
    ...
    /**
     * Make an HTTP HEAD request with rate limiting and retry logic for 429 errors and connection timeouts.
     *
     * @param  callable(): PendingRequest  $clientFactory
     * @param  array<string, mixed>  $options
     */
    public static function headWithRetry(
        ...
        while ($attempt <= $maxRetries) {
            try {
                ...
                $retryAfter = self::calculateRetryDelay($baseDelaySeconds, $attempt);
                Log::warning('Connection error during HTTP HEAD request, retrying', [
                    'url' => $url,
                    'attempt' => $attempt + 1,
                    'max_retries' => $maxRetries,
                    'retry_after' => $retryAfter,
        ...
        return $clientFactory()->head($url, $options);
    }
