...
{
    /**
     * Apply moderation rules to the current set of files, auto-disliking matches.
     *
     * @return array{filtered:Collection<int, File>, removedIds:array<int>, previewBag:array<int, array{id:int, preview:?string, title:?string}>, newlyAutoDislikedCount:int}
     */
    protected function moderateFiles(Collection $files): array
    {
        ...
        $moderationData = [];
        $filesToDelete = [];
        $metadataToUpdate = []; // Files that need moderation data but are already auto-disliked
        ...
        foreach ($files as $file) {
            ...
            if ($matchedRule) {
                $isAlreadyAutoDisliked = (bool) $file->auto_disliked;
                $hasModerationData = isset($payload['moderation']) && is_array($payload['moderation']);

                // If file is already auto-disliked but missing moderation data, populate it
                if ($isAlreadyAutoDisliked && ! $hasModerationData) {
                ...
                } elseif (! $isAlreadyAutoDisliked) {
                    // File is not auto-disliked yet, add it to matchedIds for auto-disliking
                    $matchedIds[] = $file->id;
                    $previewBag[] = [
                        'id' => $file->id,
                        'preview' => $file->thumbnail_url ?? null,
                ...
                // If file is already auto-disliked AND has moderation data, skip it
            }
        }

        // Auto-dislike new matches
        $newlyAutoDislikedCount = 0;
        if (! empty($matchedIds)) {
            // Batch update files with auto_disliked = true
            File::whereIn('id', $matchedIds)->update(['auto_disliked' => true]);
            $newlyAutoDislikedCount = count($matchedIds);
        }

        // Update moderation metadata for both newly auto-disliked files and already-auto-disliked files missing data
        ...
        if (! empty($allFileIds)) {
            try {
                ...
                Log::error('Browse moderation: Failed to persist metadata', [
                    'file_ids' => $allFileIds,
                    'exception' => $e,
                ]);
            }
        ...
        foreach ($filesToDelete as $filePath) {
            DeleteAutoDislikedFileJob::dispatch($filePath);
        }
        ...
        return [
            ...
            'newlyAutoDislikedCount' => $newlyAutoDislikedCount,
        ];
    }
}
