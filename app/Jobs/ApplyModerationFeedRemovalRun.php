...
class ApplyModerationFeedRemovalRun implements ShouldQueue
{
    ...
    public function handle(FeedRemovalBackfillService $backfill): void
    {
        ...
        if (! $run || ! in_array($run->status, [ModerationFeedRemovalRunStatus::PREVIEWED, ModerationFeedRemovalRunStatus::APPLYING], true)) {
            return;
        }

        $backfill->applyRun($run);
    }
}
