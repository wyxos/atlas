...
/**
 * Create a minimal version of items with only essential properties
 * Useful for testing if object size is the issue
 */
export function createMinimalItems<T extends Record<string, unknown>>(
    items: T[],
    essentialProperties: string[] = ['id', 'width', 'height', 'src', 'key', 'page', 'index']
): Array<Pick<T, keyof T>> {
    return items.map(item => {
        const minimal: Record<string, unknown> = {};
        for (const prop of essentialProperties) {
            if (prop in item) {
                minimal[prop] = item[prop];
            }
        }
        return minimal as Pick<T, keyof T>;
    });
}

/**
 * Compare performance between full items and minimal items
 */
export async function compareItemPerformance<T extends Record<string, unknown>>(
    fullItems: T[],
    minimalItems: Array<Pick<T, keyof T>>,
    testName = 'Item Performance Test'
): Promise<void> {
    console.group(`⚡ ${testName}`);

    // Test 1: Memory usage
    const fullSize = analyzeItemSizes(fullItems);
    const minimalSize = analyzeItemSizes(minimalItems);
    const memoryReduction = ((fullSize.totalMemoryBytes - minimalSize.totalMemoryBytes) / fullSize.totalMemoryBytes) * 100;

    console.log('Memory Comparison:');
    console.log(`Full Items: ${(fullSize.totalMemoryBytes / (1024 * 1024)).toFixed(2)} MB`);
    console.log(`Minimal Items: ${(minimalSize.totalMemoryBytes / (1024 * 1024)).toFixed(2)} MB`);
    console.log(`Reduction: ${memoryReduction.toFixed(1)}%`);

    // Test 2: Reactivity overhead (Vue-specific)
    // This is harder to measure directly, but we can estimate based on property count
    const fullPropertyCount = Object.keys(fullSize.propertyCounts).length;
    const minimalPropertyCount = Object.keys(minimalSize.propertyCounts).length;
    const reactivityReduction = ((fullPropertyCount - minimalPropertyCount) / fullPropertyCount) * 100;

    console.log('\nReactivity Overhead:');
    console.log(`Full Items Properties: ${fullPropertyCount}`);
    console.log(`Minimal Items Properties: ${minimalPropertyCount}`);
    console.log(`Reduction: ${reactivityReduction.toFixed(1)}%`);
