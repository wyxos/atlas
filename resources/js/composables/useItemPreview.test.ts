...
describe('useItemPreview', () => {
    ...
    it('updates local blacklist state from preview response', async () => {
        const item = {
            id: 1,
            width: 500,
            height: 500,
    ...
    it('clears local reaction and auto blacklist rule when preview response blacklists the item manually', async () => {
        const item = {
            id: 2,
            width: 500,
            height: 500,
            ...
            previewed_count: 2,
            auto_blacklisted: true,
            auto_blacklist_rule: { id: 1, name: 'Rule' },
            blacklisted_at: null,
        } as FeedItem;
        const items = shallowRef<FeedItem[]>([item]);
        mockQueuePreviewIncrement.mockResolvedValue({
