...
let spotifyBackgroundCheckCount = 0;
...
function startSpotifyBackgroundCheck(): void {
  ...
  spotifyBackgroundCheckCount = 0;
  spotifyBackgroundCheckId = window.setInterval(() => {
    spotifyBackgroundCheckCount++;
...
export const audioActions = {
  ...
  ensureTicker() {
    // Ensure Spotify timer is running if we're playing a Spotify track
    if (isSpotifyTrack(audioStore.currentTrack) && audioStore.isPlaying && !spotifyLastState.paused) {
      startSpotifyTimer();
    }
  },
};
