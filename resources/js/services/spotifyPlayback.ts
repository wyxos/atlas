...
export function createSpotifyPlaybackController(controllerOptions: SpotifyPlaybackOptions = {}) {
    ...
    async function prepareExpiredDeviceRegistrationRetry(
        token: string,
    ...
    return {
        ...
        async play(uri: string, positionSeconds: number, options: SpotifyPlayOptions = {}): Promise<SpotifyPlaybackSnapshot> {
            ...
            let preparedRecoveryBeforeStart = false;
            ...
            try {
                preparedRecoveryBeforeStart = await prepareExpiredDeviceRegistrationRetry(token, options, beginRecovery);
