...
describe('PageLayout', () => {
    ...
    it('leaves the app gradient visible behind page content', () => {
        const wrapper = mount(PageLayout, {
            slots: {
                default: '<div>Page content</div>',
            },
        ...
        expect(wrapper.get('[data-test="page-layout-content"]').classes()).toContain('bg-transparent');
        expect(wrapper.get('[data-test="page-layout-content"]').classes()).not.toContain('bg-prussian-blue-600');
    });
});
