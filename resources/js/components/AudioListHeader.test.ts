...
describe('AudioListHeader', () => {
    it('renders enlarged mobile controls with current-scope and library metadata actions before filtering', async () => {
        ...
        expect(wrapper.get('[data-test="audio-metadata-library-scan-cta"]').classes()).toEqual(expect.arrayContaining(['size-10', 'md:size-7']));
        ...
        expect(controls[2]?.attributes('aria-label')).toBe('Find metadata in current filter');
        expect(controls[3]?.attributes('aria-label')).toBe('Find metadata across whole library');
        expect(controls[4]?.attributes('aria-label')).toBe('Open queue');
        expect(controls[5]?.attributes('aria-label')).toBe('Filter: All');
        ...
        expect(wrapper.emitted('scanLibraryMetadata')).toHaveLength(1);
