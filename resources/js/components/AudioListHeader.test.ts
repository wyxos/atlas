...
describe('AudioListHeader', () => {
    it('renders enlarged mobile controls with a queue action before filtering', async () => {
        ...
        expect(controls[2]?.attributes('aria-label')).toBe('Open queue');
        expect(controls[3]?.attributes('aria-label')).toBe('Filter: All');
