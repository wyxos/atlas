<template>
    <div class="relative h-full">
        ...
        <div v-if="item.stats" class="absolute top-2 right-2 flex flex-col gap-1">
            <div v-if="item.stats.heartCount" class="rounded bg-black/70 px-1.5 py-0.5 text-xs text-white flex items-center gap-1">
                ❤️ {{ formatCount(item.stats.heartCount) }}
            </div>
            <div v-if="item.stats.likeCount" class="rounded bg-black/70 px-1.5 py-0.5 text-xs text-white flex items-center gap-1">
                👍 {{ formatCount(item.stats.likeCount) }}
            </div>
            <div v-if="item.stats.laughCount" class="rounded bg-black/70 px-1.5 py-0.5 text-xs text-white flex items-center gap-1">
                😂 {{ formatCount(item.stats.laughCount) }}
            </div>
            <div v-if="item.stats.cryCount" class="rounded bg-black/70 px-1.5 py-0.5 text-xs text-white flex items-center gap-1">
