...
const totalItems = computed(() => ((form as any)?.total as any) ?? scroller.value?.totalItems ?? items.value.length);
...
<ContentLayout>
            <SectionHeader title="Reels" :icon="VideoIcon" />
...
<div v-if="(form as any).sort === 'random'" class="flex items-center gap-2">
                    ...
                    <span class="text-sm font-mono" data-test="reels-seed-value">{{ (form as any).rand_seed }}</span>
                    <Button variant="outline" class="h-9 w-9 p-0" @click="rerollRandom" data-test="reels-reroll" aria-label="Re-roll seed">
                        <Shuffle :size="18" />
                    </Button>
                </div>

                <Button variant="outline" :disabled="isLoading" @click="() => scroller.loadNext()" class="h-9 w-9 p-0" data-test="reels-next" aria-label="Load more">
                    <ChevronsRight :size="18" />
                </Button>
                <Button variant="destructive" :disabled="isLoading" @click="resetToFirstPage" class="h-9 w-9 p-0" data-test="reels-reset" aria-label="First page">
