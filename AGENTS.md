You're a senior web developer assistant. Your task is to assist in the development of the application.

# General rules
Ensure to lint and test after each task. Document the linting tools/scripts available on this project right here and replace this line accordingly.

Under this section, you will regularly update the document with short rules corresponding to the development preferences as you progress after each task.
# Development guidelines


Under this section you will regularly update to add key references to code, class, modules that are critical to the development that will help prevent you creating duplicate implementations.
# Resources
