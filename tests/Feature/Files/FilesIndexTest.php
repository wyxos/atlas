...
test('files listing ignores search queries and keeps structured filters', function () {
    $admin = User::factory()->admin()->create();
    File::factory()->create(['filename' => 'test-image.jpg', 'source' => 'local']);
    File::factory()->create(['filename' => 'other-file.png', 'source' => 'local']);

    $response = $this->actingAs($admin)->getJson('/api/files?search=test-image&source=local');

    $response->assertSuccessful();

    expect($response->json('listing.items'))->toHaveCount(2);
});
...
test('admin can filter files by date range', function () {
    ...
    $response->assertSuccessful();
    expect($response->json('listing.items'))->toHaveCount(1);
});
...
test('admin can filter files by source', function () {
    ...
    $response->assertSuccessful();
    expect($response->json('listing.items'))->toHaveCount(1);
    expect($response->json('listing.items.0.source'))->toBe('local');
});
...
test('admin can filter files by downloaded status', function () {
    ...
    $response->assertSuccessful();
    expect($response->json('listing.items'))->toHaveCount(1);
    expect($response->json('listing.items.0.downloaded'))->toBeTrue();
});

test('admin receives paginated files listing ordered by last update', function () {
    $admin = User::factory()->admin()->create();
    $older = File::factory()->create(['updated_at' => now()->subDays(2)]);
    $newer = File::factory()->create(['updated_at' => now()]);
    File::factory()->count(8)->create(['updated_at' => now()->subDay()]);

    $response = $this->actingAs($admin)->getJson('/api/files?per_page=10');

    $response->assertSuccessful();
    expect($response->json('listing.items'))->toHaveCount(10);
    expect($response->json('listing.perPage'))->toBe(10);
    expect($response->json('listing.items.0.id'))->toBe($newer->id);
    expect(collect($response->json('listing.items'))->pluck('id'))->toContain($older->id);
});
