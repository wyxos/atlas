...
test('local embedded tags can use ai discogs search expansion to propose release cover', function () {
    config([
    ...
    Http::fake(function (Request $request) use (&$aiCalls, &$discogsSearches) {
        ...
        if (str_starts_with($url, 'https://discogs.test/database/search')) {
            ...
            $matchesAiQuery = ($query['release_title'] ?? null) === 'GTO Original Soundtrack 2'
                && ($query['artist'] ?? null) === 'Yusuke Homma';

            return Http::response([
                'results' => $matchesAiQuery ? [['id' => 14651911]] : [],
            ]);
        }
    ...
    $response->assertAccepted()
        ...
        ->assertJsonPath('proposal.proposed_values.artists', ['Yusuke Homma'])
