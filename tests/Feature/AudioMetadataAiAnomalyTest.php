...
test('local ai can bridge fingerprint identity to discogs original language metadata', function () {
    ...
    $response->assertAccepted()
        ...
        ->assertJsonPath('proposal.field_options.title.0.value', 'チャリンコ大捜査線')
        ->assertJsonPath('proposal.field_options.artists.0.value', ['本間勇輔'])
        ->assertJsonPath('proposal.field_options.album.0.value', 'TVアニメーション GTO オリジナルサウンドトラック')
        ->assertJsonPath('proposal.field_options.track_number.0.value', '2')
        ->assertJsonPath('proposal.field_options.release_label.0.value', 'SPE Visual Works')
        ->assertJsonPath('proposal.field_options.catalog_number.0.value', 'SVWC 1405')
        ->assertJsonPath('proposal.field_options.barcode.0.value', '4534530140548')
        ->assertJsonPath('proposal.field_options.release_date.0.value', '1999-10-21')
...
test('local ai can expand discogs search queries before reviewing a release anomaly', function () {
    ...
    $response->assertAccepted()
        ...
        ->assertJsonPath('proposal.field_options.release_label.0.value', 'Miya Records')
        ->assertJsonPath('proposal.field_options.catalog_number.0.value', 'MICA-0044')
        ->assertJsonPath('proposal.field_options.release_date.0.value', '2003')
        ->assertJsonPath('proposal.field_options.release_country.0.value', 'Taiwan')
        ->assertJsonPath('proposal.field_options.discogs_release_id.0.value', '14651911')
        ->assertJsonPath('proposal.field_options.cover_url.0.value', 'https://discogs.test/image/gto-2-primary.jpg')
...
test('local ai discogs rejects alternate compilations when current album family is stronger evidence', function () {
    ...
    $response->assertAccepted()
        ...
        ->assertJsonPath('proposal.proposed_values', [])
        ->assertJsonPath('proposal.field_options.album.0.value', 'Anjunabeats Volume Three (Mixed By Above & Beyond)')
        ->assertJsonPath('proposal.field_options.musicbrainz_release_id.0.value', 'anjunabeats-volume-three-mbid');
});
