...
it('runs the production library sync supervisor with enough workers to drain file index updates', function () {
    ...
    expect($horizon['environments']['production']['supervisor-library-sync']['minProcesses'])->toBe(6)
        ->and($horizon['environments']['production']['supervisor-library-sync']['maxProcesses'])->toBe(6)
